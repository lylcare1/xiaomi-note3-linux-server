...
依据调研结论 (见 [调研.md](./调研.md)),所需组件几乎全部已就绪:
...
**重要发现 (来自深度研究 [深度调研.md](./深度调研.md))**: 首阶段不需要任何 firmware 文件!
...
**实测结果 (2026-06-28)**: WiFi firmware 修复后 (whyred NON-HLOS.bin 替换 jason 原厂),firmware 1.0.0.591/htt-ver 3.58 工作正常,wlan0 连接 ChinaNet-810 (192.168.1.12/24),DHCP+IPv6 双栈。详见 [故障排查.md](./故障排查.md) §7.4-7.5。

### 阶段 F - 文档完善 (P1)

- [x] [docs/刷机指南.md](./刷机指南.md) - 刷写/重刷/回退流程 (2026-06-28, 8 章 417 行)
- [x] [docs/故障排查.md](./故障排查.md) - 常见问题排查 (含 WiFi 调试全过程)
...
详见 [深度调研.md §8](./深度调研.md#8-待澄清问题与已澄清答案-2026-06-27-验证) 和 [故障排查.md §1](./故障排查.md#1-已澄清问题-2026-06-27-验证完成)。
