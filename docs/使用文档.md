...
WCN3990 (UART), bluez 5.86, systemd 自动管理 `bluetooth.service`（开机约 1 分钟自动启动）。

```bash
hciconfig -a hci0                         # 状态 (UP RUNNING = 正常)
bluetoothctl show                         # D-Bus 详情 (powered/roles/UUIDs)
bluetoothctl scan on                      # BLE+BR/EDR 扫描
bluetoothctl scan off
bluetoothctl devices                      # 已发现设备列表
bluetoothctl pair <MAC>                   # 配对
bluetoothctl trust <MAC>                  # 信任
bluetoothctl connect <MAC>                # 连接

# 仅 BLE 扫描 (不需要 D-Bus, 需要 sudo)
sudo hcitool -i hci0 lescan               # 持续扫描, Ctrl-C 停止
hcitool -i hci0 name <MAC>                # 查询设备名

# 重启蓝牙服务 (systemd)
sudo systemctl restart bluetooth

# 若 hcitool 报 "I/O error" (重启后 hci0 未稳定)
sudo hciconfig hci0 down; sleep 2; sudo hciconfig hci0 up
# 或
sudo /usr/local/bin/bt-start.sh           # 幂等恢复脚本
```

设备名: `Xiaomi Mi Note 3` (bluez 从 hostname 读取), BD Address: `02:00:9E:01:40:30` (static fallback)
