...
WCN3990 (UART), bluez 5.86, 开机后约 15 秒自动可用 (server-daemon 触发 bt-start.sh)。
...
# 仅 BLE 扫描 (不需要 D-Bus)
hcitool -i hci0 lescan                    # 持续扫描, Ctrl-C 停止
hcitool -i hci0 name <MAC>                # 查询设备名

# 手动重启蓝牙 (开机自启失败时)
sudo /usr/local/bin/bt-start.sh
```

设备名: `jason-linux`, BD Address: `02:00:9E:01:40:30` (static fallback)
