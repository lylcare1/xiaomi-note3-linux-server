...
## 电池磁滞充电控制 charge-guard (2026-08-18)

### 背景

用户需求: 电量 >=60% 停止充电, <=40% 恢复充电 (电池保养, 防满电浮充鼓包)。

### 关键发现

**qcom-smbx-charger 的 `status` 属性可写** (pmOS 内核补丁暴露 SMB2 CHG_EN 位):
- `echo 0 > /sys/class/power_supply/pm660-charger/status` → 停止充电 (立即变 Discharging)
- `echo 1 > ...` → 恢复充电
- 之前文档写"mainline 无法软件限充"的结论已过时, 接口存在且实时生效

### 实现

- `charge-guard.sh` (2min timer): 磁滞逻辑 cap>=60% 停充 / cap<=40% 恢复, 中间保持
  - 状态机: /run/charge-guard-mode (holding/charging/passive), 避免重复写
  - 无外部供电时被动 arm 充电使能 (插电即充)
  - 与 discharge-monitor 低电保护不冲突 (那边管 <20%)
- `charge-guard.service/.timer` 已部署并 enable

### 首跑验证 (13:47)

```
cap=72% >= 60% → charging STOPPED (battery care) ✓
journal: charge-guard[2987]: cap=72% >= 60%, charging STOPPED
```

### 部署前全量备份 (backups/device-state-20260818/)

- device-state-backup.tar.gz: 全部脚本(13个)/systemd 单元/NM+DNS 配置/监控日志/包清单/timers 状态
- `scripts/restore-device-state.sh`: 一键恢复脚本 (上传→解压→还原→enable timers)

### 部署过程意外

- 一次 "Failed to start charge-guard.service: Transaction ... boot.mount has 'stop' job queued"
  → 设备在 13:45:49 发生了一次 reboot (journal 证实干净关机重启, 疑与 sshd 断连同因)
  → 重连后 charge-guard.timer active, 手动跑 service 成功, 未复现。已记录, 观察是否再现

## 全系统备份 (2026-08-18)

`backups/full-system-20260818/` — 337MB 四件套, 一键还原任意故障场景:

1. **rootfs.tar.gz (265MB)**: ssh 流式 GNU tar 全量 (设备端只有 BusyBox tar, 不支持 --one-file-system, 故从 Ubuntu 侧拉取)。含 13 脚本/11 timers/全部配置/监控日志
2. **boot-partition-dd.img.gz (24MB)**: mmcblk1p62 块级镜像, 可直接 fastboot flash boot
3. **boot-partition.tar.gz (48MB)**: boot 分区文件级 (boot.img/initramfs/vmlinuz/dtb)
4. **partition-metadata.txt**: 分区表/UUID/cmdline/apk world 快照
   - 关键: boot=mmcblk1p62, rootfs UUID=5a0e068c-3942-46a2-acf8-285d19520550, loop0 挂 userdata (mmcblk1p70)

README.md 含 4 种恢复场景 (SSH 在线恢复 / 重刷镜像 / fastboot 恢复 boot / 回原厂)。

### 过程问题记录

- 设备端 nohup 备份早夭 → 改 ssh 流式, 稳定
- 设备 BusyBox tar 无 --one-file-system → Ubuntu 侧 GNU tar 拉取
- apk 数据库锁 (自己第一次后台尝试留下的僵尸进程) → kill 后恢复; 但设备无外网 (USB p2p 无 NAT), openssh-sftp-server 没装成, 不影响备份
- by-name 路径实为 /dev/disk/by-partlabel/ (无 /dev/block/by-name)
- boot dd 首次 20 字节空包 (dd 错误被 2>/dev/null 吞了) → 直接指认 mmcblk1p62 重做成功

## 一键还原 + charge-guard r2 修复 (2026-08-18 下午)

### one-click-restore.sh — 真一键还原

`scripts/one-click-restore.sh`: 自动检测设备状态 (SSH 通→A / fastboot 通→B / 都不通→提示), 也可 `--mode A|A+|B|C|D|E` 手动指定。已实测自动检测路径 A 走通 (14:33 恢复 13 脚本 + 11 timer + 配置)。

| 模式 | 场景 | 动作 |
|---|---|---|
| A | SSH 可达, 配置/脚本坏 | restore-device-state.sh |
| A+ | SSH 可达, rootfs 内容坏 | 全量 rsync full-system-20260818/rootfs |
| B | SSH 不通, boot 坏 | fastboot flash boot (dd 镜像) |
| C | rootfs 彻底坏 | deploy --all + A+ |
| D | WiFi/固件坏 | fastboot flash modem (whyred) |
| E | 全失效 | 回原厂 (需输 yes 确认) |

### charge-guard r2: 修复震荡偷充电 bug

r1 缺陷: 停充后 pm660-charger/online 翻 0 → 误判"拔线" → 2min 后又放行充电 → 无限震荡。
实测: 13:47 停充时 72%, 14:38 已偷充到 83% (2min 一循环)。
r2: 移除 online 判断, 纯容量磁滞 (40-60 区间不写入)。14:52 部署, cap=85% → holding 停充。

### 设备 SSH 偶发超时

14:33-14:38 期间多次 SSH 超时后自愈 (uptime 52min 无重启)。疑似 restore 脚本连续 systemctl daemon-reload + sshd 重载所致, 待观察; 若复发查 `journalctl -u sshd`。

### one-click-restore 第二次实测 (15:10)

再次不带参数运行 `./scripts/one-click-restore.sh`: 自动检测 SSH 可达 → 路径 A, 13 脚本 + 11 timer 全部恢复启用, 30s 内完成。一键还原功能确认稳定可用。

### HANDOVER.md 全面更新 (2026-08-18)

原 HANDOVER.md 停留在 2026-07-02 (设备 halt 状态), 与现状严重脱节。已更新:
- 设备状态: 运行中, WiFi 换为 LYL
- 新增备份资产索引 (full-system-20260818 / device-state-20260818 / 原厂 28 分区 / whyred modem)
- §7.1 重写: rootfs 重刷后丢失恢复 → 一键还原 A-E 场景表
- 新增 §7.6: charge-guard 硬件停充未生效的调查现状与下一步 (regmap 直写 0x1042)
- 快速恢复决策表全部指向 one-click-restore.sh

## charge-guard 硬件停充 — 内核补丁 0010 + r3 部署 (2026-08-18 傍晚, 已完成)

用户核心需求 "电量高于60%停充, 低于40%恢复充电" **全链路打通**。

### 调查结论 (证伪 userspace 通路)
- sysfs `status=USBIN挂起`: PM660 上不停充 (r2 遗留问题)
- debugfs regmap 直写: 编译期禁用 (`CONFIG_REGMAP_DEBUGFS` 不可用)
- `current_max` 限流: 不彻底

### 方案: 内核补丁 0010-smbx-charge-behaviour.patch (pkgrel r35→r36)
给 qcom_smbx 驱动新增标准 `POWER_SUPPLY_PROP_CHARGE_BEHAVIOUR` 属性:
- 6 处 hook: 属性表 + get/set 函数 + switch case + is_writable + psy_desc charge_behaviours
- **核心发现**: `CHG_EN_SRC_BIT` (CHGR_CFG2 0x1051 bit7) 必须先置 1 (软件控制模式),
  否则 `CHARGING_ENABLE_CMD` (0x1042 bit0) 写入被硬件忽略 — init 序列默认不设此位, 是前人盲点
- pmbootstrap build 46s (ccache), 产物 qcom_smbx.ko.zst md5=5cfe139c

### 模块热替换 (无需刷机)
qcom_smbx 是可加载模块: 备份→rmmod→替换 .ko.zst→depmod→modprobe。
注意: 重启后模块从 initramfs 旧副本加载, 需再次 rmmod+modprobe 从 rootfs 加载 (已知操作)。

### 验证证据 (17:06-17:35 实测)
| 项 | inhibit 停充 | auto 恢复 |
|---|---|---|
| 1042 (CHG_EN_CMD) | 00 | 01 |
| 1051 (CHGR_CFG2) | 81 (SW控制) | 81 |
| 1006 (状态机) | 47 (DISABLE+CC_TERM) | 47 (电池满, 硬件自 inhibit) |
- cap=99% **冻结 22 分钟** → 电池净电流≈0, USB 直供系统 (若放电早掉3%, 若充电早到100%)
- 之前 "44秒被改回" 元凶 = 旧 charge-guard 服务干扰 (timer 停后 inhibit 稳定 20+min)
- `current_now` (+300mA 显示) 在此平台读数不可信, 不作判据
- 中途 16:37 设备自动 reboot = health-check WiFi 12 次失败看护动作, 与实验无关

### charge-guard.sh r3 上线
- `CHG_CTL=charge_behaviour`, 停充 `inhibit-charge`, 恢复 `auto`
- 属性存在性守卫 (缺失拒绝运行防退回)
- 17:35:32 首次动作成功: `cap=99% >= 60%, charging STOPPED (hw inhibit, battery care)`
- timer active, 2min 周期

### 遗留
- initramfs 模块同步 (可选 pmbootstrap initfs 固化)
- current_now 读数语义存疑 (不影响功能)

### 方案决策: initramfs 固化选 A (重做 initramfs), 交接至新会话执行 (17:55)

- 问题: initramfs 内 qcom_smbx 是 r35 旧版, 重启后 charge-guard 失效 (需手动 rmmod+modprobe)
- 方案 A (选定): pmbootstrap initfs + export + fastboot boot 验证 + deploy --persist, 一步到位
- 方案 B (弃): 开机自愈 oneshot 服务, 无需刷机但留旧模块窗口, 且 rootfs 方案不够干净
- 完整执行步骤/风险回退/已知坑已写入 HANDOVER.md §7.7, 新会话直接照做即可
- 附带决策: 电池 40-60% 循环需手动拔线放电一次 (当前 99% 悬浮是 inhibit 物理特性, 非 bug)

## 方案 A: initramfs 固化 qcom_smbx r36 (2026-08-18 18:31, 已完成)

问题: initramfs 内 r35 旧模块, 每次重启后 charge-guard 失效需手动 rmmod+modprobe。

### 执行中发现的关键坑 (原计划步骤不可直接用)
1. `pmbootstrap initfs` 单独跑无效 — chroot 里装的是旧内核包, 打包出来还是 r35 (24344 字节)
2. `pmbootstrap install` 不可用 — 会尝试升级上游 7.0.14 内核且网络下载失败 (Connection aborted)
3. 正确姿势: apk.static 把本地 r36 apk 直装进 chroot → initfs → export
4. export 出的 boot.img cmdline 是 debug 版 (pmos.debug-shell + loglevel=8), 必须用
   设备现役 /proc/cmdline 重新注入 (modify-bootimg-cmdline.py)

### 安全措施
- 刷前 boot 分区双备份: 设备 /tmp + 本地 backups/boot-part-20260818-pre-initfs.img (md5 5f1e1887)
- fastboot boot 临时验证通过后才 dd 持久化 (deploy.sh --flash-boot 自带 MD5+magic 校验)

### 三重验证 (全部通过)
1. fastboot boot 临时启动: charge_behaviour 属性开箱即在, 18:27:09 charge-guard 自动接管
2. 持久化后重启: srcversion 66A494BF 确认从 boot 分区加载 r36
3. 18:31:36 开机 2min: `cap=99% >= 60%, charging STOPPED (hw inhibit, battery care)` 全自动

### 结果
**重启零依赖** — 之前"重启后充电控制失效"问题彻底解决。r36 boot.img 存档 artifacts/boot-r36-20260818.img。
