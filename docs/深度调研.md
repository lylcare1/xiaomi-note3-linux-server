...
> 创建日期: 2026-06-27
> 配套文档: [调研.md](./调研.md) (高层调研) + [移植计划.md](./移植计划.md) (执行计划)
...
实测发现 jason 原厂 wlanmdsp.mbn 版本 1.0.0.533 (htt-ver 3.50) 与 mainline ath10k_snoc 驱动不兼容,启动后 ~350ms crash。解决方案: 刷入 whyred V12 完整 NON-HLOS.bin (wlanmdsp.mbn 升级到 1.0.0.591, htt-ver 3.58)。详见 [故障排查.md](./故障排查.md) §7.4。
...
下一步可以从 [移植计划.md](./移植计划.md) 阶段 A2 (pmbootstrap 环境) 开始动手实施。
...
首阶段移植 100% 成功, 所有完成定义项已达成 (详见 [故障排查.md](./故障排查.md) §7.5):
...
- ✓ 刷回/重刷流程文档 ([刷机指南.md](./刷机指南.md))

### 11.2 实测中发现的关键问题与解决

| 问题 | 根因 | 解决 | 详见 |
|---|---|---|---|
| ABL 拒绝启动 | jason DTS 缺 qcom,msm-id/board-id/pmic-id 属性 | DTS patch 添加这 3 个属性 | 故障排查.md §7.2 |
| USB gadget service failed | pmOS 自动 g1 抢占 UDC, pmos gadget 报 Resource busy | 修改脚本检测已绑定 gadget 后幂等退出 | 故障排查.md §7.6 |
| WiFi firmware crash (PC=b00c749c) | jason 原厂 wlanmdsp.mbn 1.0.0.533 与 mainline ath10k 不兼容 | 刷入 whyred V12 完整 NON-HLOS.bin (1.0.0.591) | 故障排查.md §7.4 |
| 仅替换 wlanmdsp.mbn 失败 | wlanmdsp.mbn 与 NON-HLOS.bin 其他组件版本强耦合 | 必须整包替换 NON-HLOS.bin | 故障排查.md §7.4.6 |
