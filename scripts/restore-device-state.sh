...
SSH="sshpass -p DEVICE_PASS_PLACEHOLDER ssh -o ConnectTimeout=8 -o PreferredAuthentications=password -o PubkeyAuthentication=no user@172.16.42.1"
SCP="sshpass -p DEVICE_PASS_PLACEHOLDER scp -o PreferredAuthentications=password -o PubkeyAuthentication=no"
...
echo "[2/5] 解压暂存..."
$SSH 'echo DEVICE_PASS_PLACEHOLDER | sudo -S sh -c "rm -rf /tmp/restore-stage && mkdir -p /tmp/restore-stage && tar -xzf /tmp/restore.tar.gz -C /tmp/restore-stage"'

echo "[3/5] 恢复脚本与单元..."
$SSH 'echo DEVICE_PASS_PLACEHOLDER | sudo -S sh -c "
